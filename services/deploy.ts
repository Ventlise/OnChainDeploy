 ...
 *  4. Send per-contract treasury fee
 *  5. If mode = "deploy-verify" → submit source to BaseScan
 */
 ...
 * @param gasLimit        Gas limit for this specific contract
 * @param from            Deployer wallet address
 * @param mode            "deploy" | "deploy-verify"
 * @param treasuryFeeUsd  Per-contract fee in USD — sent to treasury after deploy
...
export async function deployContract(
  ...
  // ── 4. Treasury fee — uses the exact per-contract fee passed in ───────
  ...
  // ── 5. Verification (deploy-verify only) ──────────────────────────────
  let verified = false
  let verifyMessage = ""
